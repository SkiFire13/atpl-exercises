...
#solution[
  ...
  - *Substitution lemma*: we add the following cases:
    - (T-RECORD): then $M = { ell_i = M_i #h(1pt) ^ (i in 1..n)}$ and $forall i. Gamma, x: S tack.r M_i : T_i$ with height at most $k$. By inductive hypothesis $forall i. Gamma tack.r M_i {x := N} : T_i$. By (T-RECORD) $Gamma tack.r { ell_i = M_i { x := N } #h(1pt) ^ (i in 1..n) }$
    - (T-SELECT): then $M = M'.ell_j$ and $Gamma, x: S tack.r M' : {ell_i : T_i #h(1pt) ^ (i in 1..n)}$ with height $k$ and $j in 1..n$. By inductive hypothesis $forall i. Gamma tack.r M' { x := N } : {ell_i : T_i #h(1pt) ^ (i in 1..n)}$. By (T-SELECT) $Gamma tack.r M' { x := N } .ell_j$
  ...
  - *Subject reduction*: we add the following cases:
    ...
    - (EVAL-SELECT): then $M = M'.ell_j$, $Gamma tack.r M : T_j$ derivable and $M' -> M''$ with height $k$. By inversion lemma $exists T_1..T_n. Gamma tack.r M' : { ell_i : T_i #h(1pt) ^ (i in 1..n) }$ derivable. By inductive hypothesis $Gamma tack.r M'' : { ell_i : T_i #h(1pt) ^ (i in 1..n) }$, thus by (T-SELECT) $Gamma tack.r M''.ell_j : T_j$
