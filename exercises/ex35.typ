...
#solution[
  ...
  - *Substitution lemma*: we add the following cases:
    - #T-RECORD: then $M = { ell_i = M_i #h(1pt) ^ (i in 1..n)}$, $T = { ell_i = T_i #h(1pt) ^ (i in 1..n) }$ and $forall i. Gamma, x: S tack.r M_i : T_i$ with height $<= k$. By inductive hypothesis $forall i. Gamma tack.r M_i {x := N} : T_i$. By #T-RECORD $Gamma tack.r { ell_i = M_i { x := N } #h(1pt) ^ (i in 1..n) } : { ell_i = T_i #h(1pt) ^ (i in 1..n) }$
  ...
  - *Progress theorem*: we add the following cases:
    - #T-RECORD: then $M = { ell_i = M_i #h(1pt) ^ (i in 1..n)}$ and $forall i. emptyset tack.r M_i : T_i$ with height $<= k$. By inductive hypothesis $M_i = v_i$ or $M_i -> M_i'$. We distinguish two cases:
