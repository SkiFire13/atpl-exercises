...
#solution[
  ...
  - *Subject reduction*: We add the following cases:
    - #RED-MATCH then $M = M' match { case ell_i = x_i arrow.r.double M_i #h(1pt) ^ (i in I)}$, $Gamma tack.r M : T$ and $M' arrow M''$ with height $k$, by inversion lemma $exists J subset.eq I. exists T_1, ..., T_j$ such that $Gamma tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in J) angle.r$ and $forall i in J. Gamma, x_i : T_i tack.r M_i : T$ derivable. By inductive hypothesis $Gamma tack.r M'' : angle.l ell_i : T_i #h(1pt) ^ (i in J) angle.r$ and by #TYPE-MATCH-GEN-2 $Gamma tack.r M'' match { case ell_i = x_i arrow.r.double M_i #h(1pt) ^ (i in I)} : T$
    - #MATCH then $M = angle.l ell_j = v_j angle.r match {case ell_i = x_i arrow.r.double M_i #h(1pt) ^ (i in I)}$, $j in I$ and $M -> M_j{x_j := v_j}$. By inversion lemma $exists J subset.eq I. exists T_1, ..., T_j$ such that $Gamma tack.r angle.l ell_j = v_j angle.r : angle.l ell_i : T_i #h(1pt) ^ (i in J) angle.r$ and $forall i in J. Gamma, x_i : T_i tack.r M_i : T$. By inversion lemma $exists K. exists T_1, ..., T_k.$ $j in K$ and $angle.l ell_i : T_i #h(1pt) ^ (i in J) angle.r = angle.l ell_i : T_i #h(1pt) ^ (i in K) angle.r$. Thus $J = K$ and so $j in J$. Thus $Gamma, x_j: T_j tack.r M_j : T$ and by substitution lemma $Gamma tack.r M_j{x_j := v_j} : T$
]
