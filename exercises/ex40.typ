...
#solution[
  ...
  - *Progress theorem*: We add the following case:
    - #TYPE-MATCH-GEN-2: then $M = M' match { case ell_i = x_i arrow.r.double M_i #h(1pt) ^ (i in I)}$ and $exists J subset.eq I. forall j in J. exists T_j.$ $emptyset tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in J) angle.r$ and $forall i in J. emptyset, x_i : T_i tack.r M_i : T$ all with height at most $k$.
