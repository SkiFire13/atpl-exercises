...
#solution[
  ...
  - *Progress theorem*: We add the following case:
    - (TYPE MATCH GEN 2): then $M = M' match { case l_i = x_i arrow.r.double M_i #h(1pt) ^ (i in I)}$ and by inversion lemma $exists T_1, ..., T_j$ such that $Gamma tack.r M' : angle.l l_i : T_i #h(1pt) ^ (i in J) angle.r$, $forall i in J quad Gamma, x_i : T_i tack.r M_i : T$ all with height at most $k$.
  ...
  - *Substitution lemma*: we add the following case:
    - (TYPE MATCH GEN 2) then $M = M' match { case l_i = x_i arrow.r.double M_i #h(1pt) ^ (i in I)}$ and by inversion lemma $exists T_1, ..., T_j$ such that $Gamma tack.r M' : angle.l l_i : T_i #h(1pt) ^ (i in J) angle.r$, $forall i in J quad Gamma, x_i : T_i tack.r M_i : T$ all with height at most $k$.
      We can apply the inductive hypothesis on all the premises and the rule (TYPE MATCH GEN 2) to obtain $Gamma tack.r M'{x := N} match { case l_i = x_i arrow.r.double M_i{x := N}} : T$, (note that we don't care about the branches outside of $J$).
