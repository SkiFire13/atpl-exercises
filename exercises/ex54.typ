...
#let ALGO-SUB-RECORD = smallcaps("(Algo-Sub-Record)")
#let ALGO-APP = smallcaps("(Algo-App)")
...
#solution[
  ...
  #align(center, box(prooftree(
    axiom(""),
    rule(label: BETA, $(fn x: {}. x) app { ell = 1 } -> x { x := { ell = 1 }}$),
  )))
  ...
  - $N : S$
  #align(center, box(prooftree(
    axiom(label: T-INT, $emptyset tack2r 1 : Nat$),
