...
#solution[
  ...
  - *Substitution lemma*: we add the following cases:
    - (T-VARIANT): then $M = angle.l ell_j = M_j angle.r$ and $Gamma, x: S tack.r M_j : T_j$ derivable of height $k$. By inductive hypothesis $Gamma tack.r M_j { x := N } : T_j$. By (T-VARIANT) $Gamma tack.r angle.l ell_j = M_j { x := N } angle.r$
    - (T-MATCH): then $M = M' match { case ell_i = x_i => M_i #h(1pt) ^ (i in 1..n) }$ and $exists T_1..T_n.$ $Gamma, x: S tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$, $forall i. Gamma, x: S, x_i: T_i tack.r M_i : T$ derivable with height at most $k$. By permutation lemma $forall i. Gamma, x_i: T_i, x: S tack.r M_i : T$ with the same height of at most $k$. By inductive hypothesis $Gamma tack.r M' { x := N } : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$ and $forall i. Gamma, x_i : T_i tack.r M_i { x := N } : T$. By (T-MATCH) $M' { x := N } match { case ell_i = x_i => M_i { x := N } #h(1pt) ^ (i in 1..n) }$
