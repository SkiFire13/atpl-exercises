...
#solution[
  - *Inversion of typing*: we add the following cases:
    - if $Gamma tack.r angle.l ell_j = M angle.r : T$ is derivable then $exists n, T_1...T_n. T = angle.l ell_i: T_i angle.r$ and $Gamma tack.r M : T_j$ derivable;
  ...
  - *Progress theorem*: we add the following cases:
    ...
    - (T-MATCH): then $M = M' match { case ell_i = x_i => M_i #h(1pt) ^ (i in 1..n) }$ and $exists T_1..T_n.$ $Gamma tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$, $Gamma, x_i: T_i tack.r M_i : T$ derivable with height at most $k$. By inductive hypothesis on $M'$ we distinguish two cases:
  ...
  - *Substitution lemma*: we add the following cases:
    - (T-VARIANT) then $M = angle.l ell_j = M_j angle.r$ and $Gamma, x: S tack.r M_j : T_j$ with height $k$. By inductive hypothesis $Gamma tack.r ell_j {x := N} : T_k$ and by (T-VARIANT) $Gamma tack.r M {x := N} : T$
    - (T-MATCH) the $M = M' match { case ell_i = x_i => M_i #h(1pt) ^ (i in 1..n) }$, $Gamma, x: S tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$ derivable and $Gamma, x: S, x_i : T_i tack.r M_i : T$ derivable with height at most $k$. By inductive hypothesis $Gamma tack.r M' {x := N} : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$. By permutation lemma $Gamma, x_i : T_i, x: S tack.r M_i : T$ derivable with height at most $k$. By inductive hypothesis $Gamma, x_i : T_i tack.r M_i {x := S} : T$. Thus by (T-MATCH) $Gamma tack.r M {x := N} : T$
  - *Subject reduction*: we add the following cases:
    - (MATCH): then $M = angle.l ell_j = v_j angle.r match { case ell_i = x_i => M_i #h(1pt) ^ (i in 1..n) }$ and $Gamma tack.r M : T$ derivable. By inversion lemma $exists T_1..T_n.$ $Gamma, x_i : T_i tack.r M_i : T$ and $Gamma tack.r v_j : T_j$ derivable of height at most $k$. By substitution lemma $Gamma tack.r M_j {x_j := v_j} : T$
    - (RED MATCH): then $M = M' match { case ell_i = x_i => M_i #h(1pt) ^ (i in 1..n) }$, $Gamma tack.r M : T$ derivable and $M' -> M''$ derivable with height $k$. By inversion lemma $exists T_1..T_n.$ $Gamma tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$ and $Gamma, x_i : T_i tack.r M_i : T$ derivable. By inductive hypothesis $Gamma tack.r M'' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..n) angle.r$ and by (T-MATCH) $M'' match { case ell_i = x_i => M_i #h(1pt) ^ (i in 1..n) } : T$
