...
#solution[
  ...
  - Inductive cases (height $k+1$):
    - #T-SUM: then $M = M_1 + M_2$, $ST(M) = {M} union ST(M_1) union ST(M_2)$ and $Gamma tack.r M_1 : Nat$ and $Gamma tack.r M_2 : Nat$ derivable with height $<= k$. By inductive hypothesis $forall i$ $forall M' in ST(M_i). exists Gamma', T'. Gamma' tack.r M': T'$. Thus the property holds $forall M' in ST(M)$;
    - #T-IF: then $M = mif M_1 then M_2 melse M_3$, $ST(M) = {M} union ST(M_1) union ST(M_2) union ST(M_3)$ and $Gamma tack.r M_1: Bool$, $Gamma tack.r M_2: T$ and $Gamma tack.r M_3: T$ derivable with height $<= k$. By inductive hypothesis $forall i$ $forall M' in ST(M_i). Gamma tack.r M': T'$. Thus the property holds $forall M' in ST(M)$;
    ...
    - #T-APP: then $M = M_1 app M_2$, $ST(M) = {M} union ST(M_1) union ST(M_2)$ and $Gamma tack.r M_1 : T_2 -> T$ and $Gamma tack.r M_2 : T_2$ derivable with height $<= k$. By inductive hypothesis $forall i$ $forall M' in ST(M_i). exists Gamma', T'. Gamma' tack.r M': T'$. Thus the property holds $forall M' in ST(M)$.
]
