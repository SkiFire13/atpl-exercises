...
#solution[
 ...
 - Inductive cases (height $k+1$):
    ...
    - #T-FUN: then $M = fn y : T_1. M_1 $ and $Gamma, x : T' tack.r M : T$ derivable and $x in.not fv(M) = fv(M_1) without {y}$. By $alpha$-equivalence we can assume $x != y$. From the validity of premises of (T-FUN) $exists T_2$ s.t $T = T_1 -> T_2$ and $Gamma, x : T', y : T_1 tack.r M_1 : T_2$ derivable. Since the derivation tree has height $k$ applying the inductive hypothesis $Gamma, y : T_1 tack.r M_1 : T_2$ derivable. Thus #T-FUN applies and $Gamma tack.r M : T$ derivable.
