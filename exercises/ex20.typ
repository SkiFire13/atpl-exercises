...
#solution[
 ...
 - Inductive cases (height $k+1$):
    - #T-SUM: then $M = M_1 + M_2$, and $Gamma, x: T_1 tack.r M: T$ derivable judgment and $x in.not fv(M) = fv(M_1) union fv(M_2)$ and in particular $x in.not fv(M_1)$ and $x in.not fv(M_2)$. From the validity of premises of #T-SUM, $Gamma, x: T_1 tack.r M_1: Nat$ and $Gamma tack.r M_2: Nat$ are derivable with a derivation tree of height $<= k$ and then by inductive hypotesis follows that $Gamma tack.r M_1 : Nat$ and $Gamma tack.r M_2: Nat$ are derivable. Hence, #T-SUM applies and $Gamma tack.r M: T$ is derivable; 
    - #T-IF: then $M = mif M_1 then M_2 melse M_3$, and $Gamma, x: T_1 tack.r M: T$ derivable judgment and $x in.not fv(M) = fv(M_1) union fv(M_2) union fv(M_3)$. In particular $x in.not fv(M_1)$ and $x in.not fv(M_2)$ and $x in.not fv(M_3)$. From the validity of premises of #T-IF, $Gamma, x: T_1 tack.r M_1: Bool$, $Gamma, x: T_1 tack.r M_2: T$ and $Gamma, x: T_1 tack.r M_3: T$ are derivable with a derivation tree of height $<= k$ and then by inductive hypotesis follows that $Gamma tack.r M_1 : Bool$ and $Gamma tack.r M_2 : T$ and $Gamma tack.r M_3 : T$ Hence, #T-IF applies and $Gamma tack.r M: T$ is  derivable; 
    - #T-FUN: then $M = fn y : T_1. M_1 $ and $Gamma, x : T' tack.r M : T$ derivable and since $x != y$ by $alpha-$conversion it's $x in.not fv(M) = fv(M_1) without {y}$ and then $x in.not fv(M_1)$. From the validity of premises of #T-FUN $exists T_2$ s.t $T = T_1 -> T_2$ and $Gamma, x : T', y : T_1 tack.r M_1 : T_2$ derivable, and by permutation lemma also $Gamma, y : T_1,  x : T' tack.r M_1 : T_2$ is derivable with a derivation tree of the same height $<= k$. Since the derivation tree has height $<= k$ applying the inductive hypothesis $Gamma, y : T_1 tack.r M_1 : T_2$ derivable. Thus #T-FUN applies and $Gamma tack.r M : T$ derivable.
    - #T-APP: then $M = M_1  M_2$ and $Gamma, x : T' tack.r M : T$ derivable and $x in.not fv(M) = fv(M_1) union fv(M_2)$, then $x in.not fv(M_1)$ and $x in.not fv(M_1)$. From the validity of premises of #T-APP, $exists T_1$ s.t. $Gamma, x : T' tack.r M_1 : T_1 -> T$ and $Gamma, x : T' tack.r M_2 : T_1$ are derivable. Since the derivation trees have height $<= k$ applying the inductive hypothesis we obtain that $Gamma tack.r M_1 : T_1->T$ and $M_2 : T_1$ derivable. Hence #T-APP applies and $Gamma tack.r M : T$ derivable.
]
