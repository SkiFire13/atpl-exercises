...
#solution[
 ...
 - Inductive cases (height $k+1$):
    - #T-SUM: then $M = M_1 + M_2$, and $Gamma, x: T_1 tack.r M: T$ derivable judgment of height k and $x in.not fv(M) = fv(M_1) union fv(M_2)$. From the validity of premises of (T-SUM), $Gamma, x: T_1 tack.r M_1: Nat$ and $Gamma, x: T_1 tack.r M_2: Nat$ are derivable. Hence, #T-SUM applies and $Gamma tack.r M: T$ derivable; 
    - #T-IF: then $M = mif M_1 then M_2 melse M_3$, and $Gamma, x: T_1 tack.r M: T$ derivable judgment of height k and $x in.not fv(M) = fv(M_1) union fv(M_2) union fv(M_3)$. From the validity of premises of (T-IF), $Gamma, x: T_1 tack.r M_1: Bool$, $Gamma, x: T_1 tack.r M_2: T$ and $Gamma, x: T_1 tack.r M_3: T$ are derivable. Hence, #T-IF applies and $Gamma tack.r M: T$ derivable; 
    - #T-FUN: then $M = fn y : T_1. M_1 $ and $Gamma, x : T' tack.r M : T$ derivable and $x in.not fv(M) = fv(M_1)\{x}$ then $x in.not  fv(M_1)$ and from the validity of premises of (T-FUN) $exists T_2 s.t T = T_1 -> T_1$ and $Gamma, x : T', y : T_1 tack.r M_1 : T_2$ derivable. Since the derivation tree has height $k_0$ applying the inductive hypothesis $Gamma, y : T_1 tack.r M_1 : T_2$ derivable. Thus #T-FUN applies and $Gamma tack.r M : T$ derivable. If $y = x$ we can use $alpha-$equivalence to obtain an equivalent term without the $y : fn y : T_1. M_1 =_alpha fn z : T_1 . M_1 {y:=z}$, with $z in.not Dom(Gamma,x : T')$, thus the proof then follows same steps.
    - #T-APP: then $M = M_1 M_2$ and $Gamma, x : T' tack.r M : T$ derivable and $x in.not fv(M) = fv(M_1) union fv(M_2)$, then $x in.not fv(M_1)$ and $x in.not fv(M_1)$. From the validity of premises of (T-APP), $exists T_1 s.t Gamma, x : T' tack.r M_1 : T_1 -> T$ and $Gamma, x : T' tack.r M_2 : T_1$ are derivable. Since the derivation trees have height $k_0$ applying the inductive hypothesis we obtain that $Gamma tack.r M_1 : T_1->T$ and $M_2 : T$ derivable. Hence #T-APP applies and $Gamma tack.r M : T$ derivable.
]
