...
#solution[
  ...
  - #T-SUM then $M = M_1 + M_2$, $Gamma tack.r M_i : Nat$ with height $<= k$ and $T = Nat$. We distinguish 3 cases on $M -> M'$:
  ...
  - #T-IF then $M = mif M_1 then M_2 melse M_3$, $Gamma tack.r M_1 : Bool$ and $Gamma tack.r M_{2,3}: T$ with height $<= k$. We distinguish 3 cases on $M -> M'$:
    - #IF-TRUE: thus $M -> M_2$ and we already know $Gamma tack.r M_2 : T$
    - #IF-FALSE: thus $M -> M_3$ and we already know $Gamma tack.r M_3 : T$
    - #IF: thus $M_1 -> M_1'$. By inductive hypothesis $Gamma tack.r M_1' : Bool$ and thus by #T-IF $Gamma tack.r mif M_1' then M_2 melse M_3$
  - #T-APP then $M = M_1 app M_2$, $exists T'. Gamma tack.r M_1 : T' -> T$ and $Gamma tack.r M_2 : T'$ with height $<= k$. We distinguish 3 cases on $M -> M'$:
