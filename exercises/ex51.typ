...
#solution[
  ...
  - *Substitution lemma*: we add the following case:
    - #SUBSUMPTION: then $Gamma, x: S tack.r M : U$ derivable with heigth $<= k$ and $U <: T$. By inductive hypothesis $Gamma tack.r M {x := N} : U$ derivable. By #SUBSUMPTION $Gamma tack.r M {x := N}: T$ derivable.
  - *Progress theorem*: we add the following case:
    - #T-APP: then $M = M_1 M_2$, $Gamma tack.r M_1: T_2 -> T$ and $Gamma tack.r M_2 : T_2$ with height $<= k$. By inductive hypothesis we distinguish two cases:
    ...
    - #SUBSUMPTION: then $exists S.$ $emptyset tack.r M : S$ with height $<= k$. By inductive hypothesis either $M$ is a value or $M -> M'$
