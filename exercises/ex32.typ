...
#let EVAL-PAIR-i = smallcaps("(Eval-Pair-") + "i)"
#let PROJECT-i = smallcaps("(Project-") + "i)"
#let PAIR-i = smallcaps("(Pair-") + "i)"
#let T-PROJECT-i = smallcaps("(T-Project-") + "i)"
#let T-PAIR-i = smallcaps("(T-Pair-") + "i)"
...
#solution[
  ...
  - *Progress theorem*: add the following inductive cases:
    - #T-PAIR: then $M = (M_1, M_2)$ and $emptyset tack.r M_i : T_i$ with height $<= k$. By inductive hypothesis on $emptyset tack.r M_1 : T_1$:
  ...
  - *Substitution theorem*: add the following inductive cases:
    - #T-PAIR: then $M = (M_1, M_2)$ and $Gamma, x: S tack.r M_i : T_i$ by height $<= k$. By inductive hypothesis $Gamma tack.r M_i {x := N}: T_i$ and thus by #T-PAIR $Gamma tack.r M {x := N} : T_1 * T_2$
