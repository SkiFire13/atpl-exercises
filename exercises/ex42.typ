...
#solution[
  ...
  - *Progress theorem*: We add the following case:
    - (T-MATCH DEFAULT): then $M = M' match { case ell_i = x_i arrow.r.double M_i #h(1pt) ^ (i in 1..n), default M_f }$ and by inversion lemma $exists T_1, ..., T_m$ such that $Gamma tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..m) angle.r$, $forall i in 1..n quad Gamma, x_i : T_i tack.r M_i : T$ and $Gamma tack.r M_f : T$ all with height at most $k$.
    By inductive hypothesis on $Gamma tack.r M' : angle.l ell_i : T_i #h(1pt) ^ (i in 1..m) angle.r$ we can distinguish the following cases:
