...
#solution[
  ...
  #[
    ...
    #align(center, box(prooftree(
      axiom(""),
      rule(label: (left: "MATCH DEFAULT CASE", right: $j in 1..n$), $angle.l l_j = v_j angle.r match { case l_i = x_i arrow.r.double M_i #h(1pt) ^ (i in 1..n), default M_f } arrow.r M_j {x_j := v_j}$)
    )))

    #align(center, box(prooftree(
      axiom(""),
      rule(label: (left: "MATCH DEFAULT", right: $j in.not 1..n$), $angle.l l_j = v_j angle.r match { case l_i = x_i arrow.r.double M_i #h(1pt) ^ (i in 1..n), default M_f } arrow.r M_f$)
    )))
  ]
  ...
  - *Inversion of typing*: We add the following case:
    - if $Gamma tack.r M match { case l_i = x_i arrow.r.double M_i #h(1pt) ^ (i in 1..n), default M_f } : T$ then $exists T_1, ..., T_m$ such that $Gamma tack.r M : angle.l l_i : T_i #h(1pt) ^ (i in 1..m) angle.r$, $forall i in 1..n quad Gamma, x_i : T_i tack.r M_i : T$ and $Gamma tack.r M_f : T$
  - *Canonical forms*: No new canonical form is needed.
  - *Progress theorem*: We add the following case:
    - (T-MATCH DEFAULT): then $M = M' match { case l_i = x_i arrow.r.double M_i #h(1pt) ^ (i in 1..n), default M_f }$ and by inversion lemma $exists T_1, ..., T_m$ such that $Gamma tack.r M' : angle.l l_i : T_i #h(1pt) ^ (i in 1..m) angle.r$, $forall i in 1..n quad Gamma, x_i : T_i tack.r M_i : T$ and $Gamma tack.r M_f : T$ all with height at most $k$.
