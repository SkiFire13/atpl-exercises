...
#solution[
  ...
  - *Type Preservation Theorem*: We add the following cases:
    ...
    - (RAISE IFELSE): $mif throw v then M melse N -> throw v$, by inversion lemma $Gamma tack.r throw v : Bool$, by inversion lemma again $Gamma tack.r v : T_exn$ and so by (T-RAISE) $Gamma tack.r throw v : T$
  ...
  - *Progress Theorem*: Let $M$ be a closed and well-typed term, i.e. $emptyset tack.r M : T$ then either $M$ is a value, or $exists M'.M -> M'$ or $exists v. M = throw v$. We add the following cases:
    ...
    - (T-TRY): $M = try M_1 catch N$ and $emptyset tack.r try M_1 catch N : T$ has a derivation tree of height k+1 while $emptyset tack.r M_1 : T$ has a derivation tree of height k. By inductive hypothesis on $M_1$ we distinguish three cases:
